 ...
 */
public class UpperAntiDiagSelecter extends View {
    ...
    public UpperAntiDiagSelecter(Context context, AttributeSet attrs) {
        super(context, attrs);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (myMatrix.getZoom()) {
            // Activ
            if (myMatrix.getBlockSelecterMatrix()[5][myMatrix.getSelectedBlockNumber()]) {
                canvas.drawLine(linePitch, height - 2 * linePitch, height - 2 * linePitch, linePitch, activePaint);
            }
            //No Active
            else {
                canvas.drawLine(linePitch, height - 2 * linePitch, height - 2 * linePitch, linePitch, deactivePaint);
            }
        } else {
            // Activ
            if (myMatrix.getWholeSelecterVector()[5]) {
                canvas.drawLine(linePitch, height - 2 * linePitch, height - 2 * linePitch, linePitch, activePaint);
            }
            //No Active
            else {
    ...
    public boolean onTouchEvent(MotionEvent event) {
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN:
                ...
                if (myMatrix.getZoom() == true && myMatrix.getBlockSelecterMatrix()[5][myMatrix.getSelectedBlockNumber()] == false) {
                    myMatrix.selectUpperAntiDiag(myMatrix.getSelectedBlockNumber());
                    if (!matrixView.getIsSolving()) {
                        computeButton.setVisibility(View.INVISIBLE);
                    }
                } else if (myMatrix.getZoom() == true && myMatrix.getBlockSelecterMatrix()[5][myMatrix.getSelectedBlockNumber()] == true) {
                    myMatrix.unselectUpperAntiDiag(myMatrix.getSelectedBlockNumber());
                    if (!matrixView.getIsSolving()) {
                        computeButton.setVisibility(View.INVISIBLE);
                    }
                } else if (myMatrix.getZoom() == false && !myMatrix.getWholeSelecterVector()[5]) {
                    myMatrix.selectUpperAntiDiag(-1);
                    if (!matrixView.getIsSolving()) {
                        computeButton.setVisibility(View.INVISIBLE);
                    }
                } else if (myMatrix.getZoom() == false && myMatrix.getWholeSelecterVector()[5]) {
                    myMatrix.unselectUpperAntiDiag(-1);
                    if (!matrixView.getIsSolving()) {
                        computeButton.setVisibility(View.INVISIBLE);
                    }
                }
