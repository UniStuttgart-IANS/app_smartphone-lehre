...
public class PurePoints {
    //
    private int count;
    private int[] sort;
    private ArrayList<PurePoint> pointList;

    public PurePoints() {
        this(0);
    }

    public PurePoints(int count) {
        this.count = 0;
        pointList = new ArrayList<PurePoint>();
    ...
    public PurePoint addPoint() {
        this.pointList.add(new PurePoint(count));
        this.count++;

        return pointList.get(count - 1);
    }

    public PurePoint getPoint(int number) {
        return pointList.get(number);
    }


    public PurePoint getActive() {
        int flag = 0;
        for (int i = 0; i < count; i++) {
