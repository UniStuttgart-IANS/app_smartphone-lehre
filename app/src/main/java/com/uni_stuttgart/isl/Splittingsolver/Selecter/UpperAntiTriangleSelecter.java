 ...
 */
public class UpperAntiTriangleSelecter extends View {
    ...
    public UpperAntiTriangleSelecter(Context context, AttributeSet attrs) {
        super(context, attrs);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (myMatrix.getZoom()) {
            // Activ
            if (myMatrix.getBlockSelecterMatrix()[9][myMatrix.getSelectedBlockNumber()]) {
                canvas.drawLine(linePitch, height - 2 * linePitch, width - 2 * linePitch, linePitch, activePaint);
                canvas.drawLine(linePitch, height - 2 * linePitch, linePitch, linePitch, activePaint);
                canvas.drawLine(linePitch, linePitch, width - 2 * linePitch, linePitch, activePaint);
            }
            //No Active
            else {
                canvas.drawLine(linePitch, height - 2 * linePitch, width - 2 * linePitch, linePitch, deactivePaint);
                canvas.drawLine(linePitch, height - 2 * linePitch, linePitch, linePitch, deactivePaint);
                canvas.drawLine(linePitch, linePitch, width - 2 * linePitch, linePitch, deactivePaint);
            }
        } else {
            // Activ
            if (myMatrix.getWholeSelecterVector()[9]) {
                canvas.drawLine(linePitch, height - 2 * linePitch, width - 2 * linePitch, linePitch, activePaint);
                canvas.drawLine(linePitch, height - 2 * linePitch, linePitch, linePitch, activePaint);
                canvas.drawLine(linePitch, linePitch, width - 2 * linePitch, linePitch, activePaint);
            }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN:
                ...
                if (myMatrix.getZoom() == true && myMatrix.getBlockSelecterMatrix()[9][myMatrix.getSelectedBlockNumber()] == false) {
                    myMatrix.selectUpperAntiTriangle(myMatrix.getSelectedBlockNumber());
                    if (!matrixView.getIsSolving()) {
                        computeButton.setVisibility(View.INVISIBLE);
                    }
                } else if (myMatrix.getZoom() == true && myMatrix.getBlockSelecterMatrix()[9][myMatrix.getSelectedBlockNumber()] == true) {
                    myMatrix.unselectUpperAntiTriangle(myMatrix.getSelectedBlockNumber());
                    if (!matrixView.getIsSolving()) {
                        computeButton.setVisibility(View.INVISIBLE);
                    }
                } else if (myMatrix.getZoom() == false && !myMatrix.getWholeSelecterVector()[9]) {
                    myMatrix.selectUpperAntiTriangle(-1);
                    if (!matrixView.getIsSolving()) {
                        computeButton.setVisibility(View.INVISIBLE);
                    }
                } else if (myMatrix.getZoom() == false && myMatrix.getWholeSelecterVector()[9]) {
                    myMatrix.unselectUpperAntiTriangle(-1);
                    if (!matrixView.getIsSolving()) {
                        computeButton.setVisibility(View.INVISIBLE);
                    }
                }
