 ...
 */
public class LowerDiagSelecter extends View {
    ...
    public LowerDiagSelecter(Context context, AttributeSet attrs) {
        super(context, attrs);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (myMatrix.getZoom()) {
            // Activ
            if (myMatrix.getBlockSelecterMatrix()[2][myMatrix.getSelectedBlockNumber()]) {
            ...
            // Activ
            if (myMatrix.getWholeSelecterVector()[2]) {
                canvas.drawLine(linePitch, 2 * linePitch, width - 2 * linePitch, height - linePitch, activePaint);
            }
            //No Active
            else {
    ...
    public boolean onTouchEvent(MotionEvent event) {
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN:
                ...
                if (myMatrix.getZoom() == true && myMatrix.getBlockSelecterMatrix()[2][myMatrix.getSelectedBlockNumber()] == false) {
                ...
                } else if (myMatrix.getZoom() == true && myMatrix.getBlockSelecterMatrix()[2][myMatrix.getSelectedBlockNumber()] == true) {
                    myMatrix.unselectLowerDiag(myMatrix.getSelectedBlockNumber());
                    if (!matrixView.getIsSolving()) {
                        computeButton.setVisibility(View.INVISIBLE);
                    }
                } else if (myMatrix.getZoom() == false && !myMatrix.getWholeSelecterVector()[2]) {
