...
public class CoordSystem {
    ...
    private TextView middelPointErrorView;
    private TextView trapezErrorView;
    private TextView gaussErrorView;
    private double[] yGauss;
    private double[] yGaussTrans;
    ...
    public void drawSimpsonFunc() {
        boolean flag = false;
        for (int i = 1; i < getCoordArtFlag() * numXPix + 1; i=i+5) {
            if ((ySimpsonTrans[i] >= -1 || ySimpsonTrans[i - 1] >= -1) && (ySimpsonTrans[i] <= 1201 || ySimpsonTrans[i - 1] <= 1201)) {
                myCanvas.drawSimpsonFunc((float) xFuncTrans[i - 1], (float) ySimpsonTrans[i - 1], (float) xFuncTrans[i], (float) ySimpsonTrans[i], flag);
                flag = true;
            }
        }
    }

    public void drawGaussQuadraturFunc() {
        boolean flag = false;
        for (int i = 1; i < getCoordArtFlag() * numXPix + 1; i=i+5) {
            if ((yGaussTrans[i] >= -1 || yGaussTrans[i - 1] >= -1) && (yGaussTrans[i] <= 1201 || yGaussTrans[i - 1] <= 1201)) {
                myCanvas.drawGaussQuadraturFunc((float) xFuncTrans[i - 1], (float) yGaussTrans[i - 1], (float) xFuncTrans[i], (float) yGaussTrans[i], flag);
                flag = true;
            }
        }
    }

    public void setySimpson(Function simpsonInterpolating) {
         // Anlegen der entsprechenden Vektoren (Länge: 2*numXPix, da positive UND negate y-Achse)
        ySimpson = new double[2 * numXPix + 1];
        for (int i = 0; i < 2 * numXPix + 1; i++) {
            ...
            ySimpson[i] = simpsonInterpolating.evaluate(xFunc[i]);
        }

        // Anlegen der entsprechenden Vektoren (Länge: 2*numXPix, da positive UND negate y-Achse)
        ySimpsonTrans = new double[2 * numXPix + 1];
        for (int i = 0; i < 2 * numXPix + 1; i++) {
            // yFunc enthält die "echten" y-Werte, während yFuncTrans die auf das Koordiantensystem gemappten Werte enthält.
            // Hierbei gilt: Pro Pixel/x-Wert = ein y-Wert
            ySimpsonTrans[i] = -(ySimpson[i] * yScale) + y0;
         }
    }

    public void setyGauss(Function gaussQuadraturInterpolating) {
        // Anlegen der entsprechenden Vektoren (Länge: 2*numXPix, da positive UND negate y-Achse)
        yGauss = new double[2 * numXPix + 1];
        for (int i = 0; i < 2 * numXPix + 1; i++) {
            // yFunc enthält die "echten" y-Werte, während yFuncTrans die auf das Koordiantensystem gemappten Werte enthält.
            // Hierbei gilt: Pro Pixel/x-Wert = ein y-Wert
            yGauss[i] = gaussQuadraturInterpolating.evaluate(xFunc[i]);
        }

        // Anlegen der entsprechenden Vektoren (Länge: 2*numXPix, da positive UND negate y-Achse)
        yGaussTrans = new double[2 * numXPix + 1];
        for (int i = 0; i < 2 * numXPix + 1; i++) {
            // yFunc enthält die "echten" y-Werte, während yFuncTrans die auf das Koordiantensystem gemappten Werte enthält.
            // Hierbei gilt: Pro Pixel/x-Wert = ein y-Wert
            yGaussTrans[i] = -(yGauss[i] * yScale) + y0;
        }
    }
    ...
    public double[] getySimpsonTrans() {
        return ySimpsonTrans;
    }
    ...
    public void setSimpsonErrorView(TextView simpsonErrorView) {
        this.simpsonErrorView = simpsonErrorView;
    }

    public TextView getSimpsonErrorView() {
        return simpsonErrorView;
    }

    public TextView getStartPointErrorView() {
        return startPointErrorView;
    }

    public TextView getMiddelPointErrorView() {
        return middelPointErrorView;
    }

    public TextView getTrapezErrorView() {
        return trapezErrorView;
    }

    public void setStartPointErrorView(TextView startPointErrorView) {
        this.startPointErrorView = startPointErrorView;
    }

    public void setMiddlePointErrorView(TextView middelPointErrorView) {
        this.middelPointErrorView = middelPointErrorView;
    }

    public void setTrapezErrorView(TextView trapezErrorView) {
        this.trapezErrorView = trapezErrorView;
    }

    public TextView getGaussErrorView() {
        return gaussErrorView;
    }

    public void setGaussErrorView(TextView gaussErrorView) {
        this.gaussErrorView = gaussErrorView;
    }
